...
| Серьёзность  | ID     | Проблема                             | Файлы                                          |
| ------------ | ------ | ------------------------------------ | ---------------------------------------------- |
| **CRITICAL** | C1     | Нет аутентификации                   | Все 16 файлов actions                          |
| **CRITICAL** | C2     | Нет авторизации (RBAC)               | Все 16 файлов actions                          |
| **CRITICAL** | C3     | Prisma query injection               | 12+ функций get\*                              |
| **CRITICAL** | C4     | Нет tenant isolation                 | Все CRUD + `@default(1)`                       |
| **CRITICAL** | C5     | Unsafe file upload                   | `products.ts`                                  |
| **CRITICAL** | C6     | Нет Error Boundaries                 | Все маршруты `src/app/`                        |
| **CRITICAL** | C7     | `next/router` в App Router           | `no-organization/page.tsx`                     |
| **CRITICAL** | C8     | Двойное хеширование пароля           | `member-create-mutation.ts`                    |
| **CRITICAL** | C9     | Пароли студентов в открытом виде     | `schema.prisma`, `students-table.tsx`          |
| **MAJOR**    | M1     | Race condition (no tx)               | `orders.ts`, `groups.ts`                       |
| **MAJOR**    | M2     | Unsafe `members[0]`                  | 15+ файлов                                     |
| **MAJOR**    | M3     | Нет валидации входных данных         | Все actions                                    |
| **MAJOR**    | M4     | Auth-проверка дублирована            | 15+ страниц                                    |
| **MAJOR**    | M5     | Нет индексов на FK                   | `schema.prisma` (10+ полей)                    |
| **MAJOR**    | M6     | `Category @unique` глобально         | `schema.prisma`                                |
| **MAJOR**    | M7     | Waterfall-запросы                    | 5+ страниц                                     |
| **MAJOR**    | M8     | `Float` для денег                    | `schema.prisma`                                |
| **MAJOR**    | M9     | Одинаковый queryKey для permissions  | `organization-permission-query.ts`             |
| **MAJOR**    | M10    | Тяжёлые запросы без агрегации        | `attendance.ts`, `dismissed.ts`, `students.ts` |
| **MAJOR**    | M11    | Type mismatch `FindFirst/findMany`   | `dismissed.ts`                                 |
| **MAJOR**    | M12    | Дублирование типа `StudentWith...`   | `types/student.ts`, `actions/students.ts`      |
| **MAJOR**    | M13    | Unrestricted update `attendance`     | `attendance.ts`                                |
| **MAJOR**    | M14    | Дублирование `Filters` компонента    | `dashboard.tsx`, `groups-table.tsx`            |
| **MAJOR**    | M15    | Дублирование `DataTable`             | `data-table.tsx`, `dashboard.tsx`              |
| **MAJOR**    | M16    | Смешение actions и inline prisma     | 6+ страниц                                     |
| **MAJOR**    | M17    | `organizationId!` non-null assertion | `dashboard.tsx`                                |
| **MAJOR**    | M18    | Пароль в таблице                     | `students-table.tsx`                           |
| **MINOR**    | m1-m24 | UI/нейминг/стиль/copy-paste          | Разные файлы                                   |
...
3. **Принудительно добавлять `organizationId`** из сессии во все запросы. Убрать `@default(1)`.

4. **Валидировать файлы:** whitelist расширений (`.jpg`, `.png`, `.webp`), проверка MIME-type, ограничение размера.

5. **Убрать `hashPassword` из клиента.** Перенести создание пользователя в server action.

6. **Хешировать пароли студентов.** Убрать колонку `password` из UI.

### Приоритет 2 — Целостность данных (M1, M5–M6, M8–M9)

1. Обернуть связанные операции в `prisma.$transaction`
2. Добавить `@@index` на все FK-поля
3. `Category`: `@@unique([organizationId, name])`
4. `Product.price`: `Decimal` или `Int`
5. Включить `permission` в queryKey

### Приоритет 3 — Архитектура (C6–C7, M4, M7, M10)

1. Добавить `global-error.tsx`, `error.tsx`, `loading.tsx`
2. Исправить `next/router` → `next/navigation`
3. Вынести auth-проверку в middleware
4. Параллелизировать запросы через `Promise.all`
5. Заменить in-memory агрегации на SQL-агрегации

### Приоритет 4 — Качество кода (M11–M18, m1–m24)

1. Унифицировать язык (i18n или единый язык)
2. Вынести типы из `'use server'` файлов
3. Устранить дублирование компонентов и типов
4. Убрать mock-функции
5. Исправить тайпо, нейминг, revalidatePath
